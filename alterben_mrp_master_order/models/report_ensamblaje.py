...
class ReportEnsamblaje(models.AbstractModel):
    _name = 'report.alterben_mrp_master_order.report_ensamblaje'
    _description = 'Reporte Ensamblaje (backend)'
    ...
    def _get_lines(self, master):
        ...
        # Prefer specific tab if exists, fallback to all lines
        lines = getattr(master, 'line_ids_ensamblaje', None)
        if lines is None:
            # some setups might name it ensamblado
